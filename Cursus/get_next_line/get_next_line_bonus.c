...
/*   Updated: 2025/10/26 17:43:50 by abalcu           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (buf[fd].data == NULL)
	{
		buf[fd].capacity = CHUNK;
		buf[fd].data = (char *)ft_calloc(buf[fd].capacity, sizeof(char));
		if (buf[fd].data == NULL)
			return (NULL);
	}
	if (!read_into_buffer(&buf[fd], fd))
	{
		if (buf[fd].data != NULL)
			free_buf(&buf[fd]);
		return (NULL);
